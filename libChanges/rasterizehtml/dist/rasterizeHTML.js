    ...
    var supportsReadingObjectFromCanvas = function (url) {
        return new Promise(function (resolve, reject) {
            ...
            image.onload = function () {
                ...
                try {
                    context.drawImage(image, 0, 0); //TITI change
                    //context.drawImage(image,sx,sy,swidth,sheight,0,0,width,height);//TODO CLIP THE IMAGE
...
var document2svg = (function (util, browser, documentHelper, xmlserializer) {
    ...
    var svgAttributes = function (size, options) {
        var zoomFactor = options.zoom || 1;
        ...
        if (zoomFactor !== 1 || offset.x!=0 || offset.y!=0) {
            attributes.style = 'transform:scale(' + zoomFactor + '); transform-origin: '+offset.x+'px '+offset.y+'px;';
        }
    ...
    var convertElementToSvg = function (element, size, options) {
        ...
        return (
            '<svg xmlns="http://www.w3.org/2000/svg"' +
                serializeAttributes(svgAttributes(size, options)) +
                '>' +
    ...
    module.getSvgForDocument = function (element, size, options) {
        documentHelper.rewriteTagNameSelectorsToLowerCase(element);

        return convertElementToSvg(element, size, options);
    };
    ...
    module.drawDocumentAsSvg = function (element, options) {
        ...
        return browser.calculateDocumentContentSize(element, options)
            .then(function (size) {
                return module.getSvgForDocument(element, size, options);
            });
    };
